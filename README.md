<!-- Terminal Header -->
<div align="left">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=30363d&height=2&width=100%" />
  <h3>🔴 🟡 🟢  ~/identity/Adityanrrhm</h3>
</div>

<!-- Typing Intro -->
<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=20&duration=2800&pause=1000&color=D4D4D4&background=00000000&vCenter=true&width=600&lines=%24+whoami;>+Data+Engineer+%26+CS+Student;>+Data+is+the+fuel%2C+pipelines+are+the+engine;>+Building+robust+%26+scalable+data+flows;>+Simplicity+is+my+compass;>+Still+learning%E2%80%94always+will+be" alt="Typing SVG" />
</a>

<!-- Social Links - Steel Theme -->
<p>
  <a href="https://github.com/Adityanrrhm"><img src="https://img.shields.io/badge/GitHub-Adityanrrhm-64748B?style=flat-square&logo=github&logoColor=white&labelColor=1E293B" height="22"/></a>
  <a href="https://linkedin.com/in/adityanrhm"><img src="https://img.shields.io/badge/LinkedIn-connect-64748B?style=flat-square&logo=linkedin&logoColor=white&labelColor=1E293B" height="22"/></a>
  <a href="https://instagram.com/adtyanrhm"><img src="https://img.shields.io/badge/Instagram-follow-64748B?style=flat-square&logo=instagram&logoColor=white&labelColor=1E293B" height="22"/></a>
</p>


### ` $ cat profile.json `

```json
{
  "name": "Aditya Nur Rohim",
  "title": "Data Engineer / Specialist",
  "location": "Sidoarjo, East Java 🇮🇩",
  "education": "Computer Science Student",
  
  "experience": [
    "Building & optimizing ETL/ELT pipelines",
    "Data warehousing & dimensional modeling",
    "Database design & query performance optimization"
  ],
  
  "expertise": [
    "Python", "SQL", "PostgreSQL",
    "ETL/ELT", "Docker", "Linux"
  ],
  
  "learning": [
    "Apache Spark", "Airflow", "Cloud Data Platforms"
  ],
  
  "available": true
}
```

<br/>

### ` $ cat ~/skills `

<p>
  <img src="https://skillicons.dev/icons?i=python,r,pandas,numpy,postgres,mysql,tableau,git,github&theme=dark" />
</p>

### ` $ fortune | cowsay `

<p>
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=dark" />
</p>

### ` $ cat ~/achievements `

<p>
  <img src="https://github-profile-trophy.vercel.app/?username=Adityanrrhm&theme=darkhub&no-frame=true&no-bg=true&column=4&margin-w=15&margin-h=15" />
</p>

### ` $ ls -l ./projects `

| Mode | Name | Tech | Description |
| :--- | :--- | :--- | :--- |
| `drwxr-xr-x` | [**kas-management**](https://github.com/Adityanrrhm/kas-management) | ![Laravel](https://img.shields.io/badge/Laravel-64748B?style=flat-square&logo=laravel&logoColor=white&labelColor=1E293B) | Financial transaction strategy system |
| `drwxr-xr-x` | [**softfixer**](https://github.com/Adityanrrhm/softfixer) | ![PHP](https://img.shields.io/badge/PHP-64748B?style=flat-square&logo=php&logoColor=white&labelColor=1E293B) | Service management utility tool |

<br/>

### ` $ cat ~/stats `

<p>
  <img src="https://github-stats-extended.vercel.app/api?username=Adityanrrhm&show_icons=true&hide_border=true&bg_color=0D1117&title_color=94A3B8&text_color=64748B&icon_color=94A3B8" height="165" />
  <img src="https://streak-stats.demolab.com/?user=Adityanrrhm&hide_border=true&background=0D1117&stroke=1E293B&ring=94A3B8&fire=94A3B8&currStreakNum=64748B&sideNums=64748B&currStreakLabel=94A3B8&sideLabels=64748B&dates=475569" height="165" />
</p>

<p>
  <img src="https://github-stats-extended.vercel.app/api/top-langs/?username=Adityanrrhm&layout=compact&hide_border=true&bg_color=0D1117&title_color=94A3B8&text_color=64748B&hide=html,css" height="140" />
</p>

### ` $ tail -f ~/activity.log `

<p>
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Adityanrrhm&bg_color=0D1117&color=64748B&line=94A3B8&point=CBD5E1&area=true&area_color=1E293B&hide_border=true" width="100%" />
</p>

<br/>

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Adityanrrhm/Adityanrrhm/output/github-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Adityanrrhm/Adityanrrhm/output/github-snake.svg" />
    <img alt="github-snake" src="https://raw.githubusercontent.com/Adityanrrhm/Adityanrrhm/output/github-snake-dark.svg" width="100%" />
  </picture>
</div>

<br/>

<div align="center">
  <code>sender: adityanrrhm@github ~ $ exit 0_</code>
</div>
